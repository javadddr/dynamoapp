...
function NoteCar({ theme, car: propCar, onCarUpdate }) {
  const [car, setCar] = useState(propCar);
  const [newNoteContent, setNewNoteContent] = useState('');
  const isDarkMode = theme === 'dark';
  useEffect(() => {
    setSortedNotes([...car.notes].sort((b, a) => new Date(a.creatingDate) - new Date(b.creatingDate)));
  }, [car.notes]);
  ...
  const handleEdit = (note) => {
    ...
    const handleSave = async note => {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/cars/${car._id}/notes/${editingNoteId}`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`,
  ...
  const handleAddNote = async () => {
    ...
    const username = localStorage.getItem('username');
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/cars/${car._id}/notes`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
        ...
        return (
          <Card key={note._id} isHoverable variant="shadow" style={{marginLeft:"25px", marginRight:"25px",marginBottom: '15px', display: 'flex' }}>
            ...
            <CardHeader className="text-xs" style={{ backgroundColor: '#FFF6D6', padding: '4px',justifyContent:"flex-end" }}>
              <div className="mr-3 flex flex-end text-slate-400">Created by: {creatorName} -{dayjs(note.creatingDate).format('DD MMM HH:mm')}</div>
            </CardHeader>
