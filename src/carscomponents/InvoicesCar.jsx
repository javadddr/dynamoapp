...
function InvoicesCar({theme, car: propCar, onCarUpdate}) {
  ...
  cars.forEach(car => {
    ...
    const carInvoiceSum = invoices.reduce((sum, invoice) => sum + invoice.amount, 0);
    const carInvoiceCount = invoices.length;
    
    totalInvoiceSum += carInvoiceSum;
    totalInvoiceCount += carInvoiceCount;
    
    const carAverage = carInvoiceSum / carInvoiceCount;

    
    // Track max and min car
    if (carAverage > maxAverage) {
      maxAverage = carAverage;
      maxCar = car;
    }
  
    if (carAverage < minAverage) {
      minAverage = carAverage;
      minCar = car;
    }
  });
  
  const totalAverageInvoice = totalInvoiceSum / totalInvoiceCount;
  const invoicesi = car.invoices;
...
useEffect(() => {
  ...
  const addInvoice = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/cars/${car._id}/invoices`, {
        method: 'POST',
        headers: {
  ...
  // Example usage
  const chartData = getChartDataByMonth(car.invoices);
  ...
  const confirmDeleteInvoice = async () => {
    ...
    const token = localStorage.getItem('userToken');
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/cars/${car._id}/invoices/${invoiceToDelete}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
